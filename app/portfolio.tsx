  ...
  return <main className="portfolio">
    ...
    <section className="experience" aria-label="Explore five studio films">
      ...
      <div className={`world-frame ${ready && !lite ? 'is-ready' : ''}`}>
        ...
        {mounted && !lite && <World selected={selected} calm={calm} playing={player} reset={reset} onSelect={select} onPlay={() => enter()} onReady={setReady} onInteract={() => setInteracted(true)} />}
      </div>
      <p className={`mobile-hint ${interacted ? 'hint-dismissed' : ''}`}>Swipe to explore · Tap a film to watch</p><div className="film-caption"><p className="eyebrow">{String(selected + 1).padStart(2, '0')} / 05 <span>— {film.category}</span></p><h2>{film.title}</h2><p>{film.description}</p></div>
